...
export default function EscalasPage() {
  ...
  // Estado para os botões dinâmicos de dias da semana
      ...
      <div className="p-4 md:p-8 max-w-6xl mx-auto animate-fade-in pb-20 relative">
        ...
        {carregando ? (
          ...
          <div className="space-y-16">
            {tiposExistentes.map((tipo, index) => {
              const cor = paletasDeCores[index % paletasDeCores.length];
              const escalasDoTipo = escalasAgrupadas[tipo];
              
              // Inteligência para descobrir quais dias da semana únicos existem nesse tipo
              const diasMap = new Map();
              escalasDoTipo.forEach((e: any) => {
                const nomeDia = capitalizar(format(parseISO(e.data), "EEEE", { locale: ptBR }));
                diasMap.set(nomeDia, parseISO(e.data).getDay()); // getDay = 0 a 6 (Dom a Sáb)
              });
              
              // Organiza os botões cronologicamente na semana
              const diasUnicos = Array.from(diasMap.keys()).sort((a, b) => diasMap.get(a) - diasMap.get(b));
              
              // Estado do filtro atual para esse tipo (Padrão: Todos)
              const filtroDiaAtual = filtrosDia[tipo] || 'Todos';
              
              // Filtra os quadros que serão exibidos
              const escalasFiltradas = escalasDoTipo.filter((e: any) => {
                 if (filtroDiaAtual === 'Todos') return true;
                 return capitalizar(format(parseISO(e.data), "EEEE", { locale: ptBR })) === filtroDiaAtual;
              });

              return (
                <section key={tipo} id={criarIdAncora(tipo)} className="scroll-mt-40">
                  
                  {/* CABEÇALHO DA SEÇÃO COM OS FILTROS DE DIA */}
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b-2 border-gray-100 pb-4">
                    ...
                    {diasUnicos.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        <button 
                          onClick={() => setFiltrosDia({...filtrosDia, [tipo]: 'Todos'})}
                          className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${filtroDiaAtual === 'Todos' ? `${cor.base} text-white shadow-md transform scale-105` : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                        >
                          Todos
                        </button>
                        {diasUnicos.map(dia => (
                          <button 
                            key={dia}
                            onClick={() => setFiltrosDia({...filtrosDia, [tipo]: dia})}
                            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${filtroDiaAtual === dia ? `${cor.base} text-white shadow-md transform scale-105` : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                          >
                            {dia}
                          </button>
                        ))}
                  ...
                  {escalasFiltradas.length === 0 ? (
                    <div className="py-8 text-center text-gray-400 text-sm border-2 border-dashed border-gray-100 rounded-xl">
                      Nenhuma escala encontrada para esse dia da semana.
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {escalasFiltradas.map((escala: any) => (
                        <div key={escala.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow flex flex-col group animate-fade-in">
                          ...
                          {escala.descricao && (
                            <div className={`px-5 py-3 ${cor.light} border-b ${cor.border} shrink-0`}>
                              ...
                              <div className={`text-sm font-semibold italic ${cor.text} whitespace-pre-wrap break-words`}>
                                {renderComLinks(escala.descricao)}
                              </div>
                            </div>
                          )}
